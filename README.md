...
- **Releases**：发布版本入口，每个 tag 对应一次发布。**注意：Release 资源里不一定带源码**，多数时候只有编译好的二进制，源码是否随包上传看具体那次发布
- **仓库目录**：部分工具源码会直接放在仓库对应子目录（如 `lns/`、`build/`），源码基本都在仓库里，只是未必每次打包进 Release
- **文件本身**：`file` / `--version` / `--help` 自己看

> 💡 想找源码：优先在仓库文件树里翻；Release 只是拿二进制最快的地方。

## 🏷️ 版本节奏

- 新工具、大改动 → 递增主/次版本号，单独发 Release
- Release 里**优先放编译好的 ELF 二进制**，源码以仓库内目录为准，不保证每个 Release 都带
